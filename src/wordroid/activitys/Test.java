...
public class Test extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setTitle("TESTLIST-"+wordListInt);
		...
		this.attention.setOnClickListener(new Button.OnClickListener(){
			...
			public void onClick(View v) {
				...
				if (attention.size()==0){
					...
					Toast.makeText(Test.this, "Added to your list successfully", Toast.LENGTH_SHORT).show();
				}
				else Toast.makeText(Test.this, "Already Existed£¡", Toast.LENGTH_SHORT).show();
			}
		...
		overBtn.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				.setTitle("TESTING RESULT")
				.setMessage("TOTAL" + listLength + "Questions£¬Correct answers" + score + "Questions£¬ Percentage" + (score*100/listLength) + "%")
					...
					switch (rightAns) {
						case 0:
							if ( checkedId == meaning0.getId() ) {
								...
								DisplayToast("WRONG,THE CORRECT ANSWER IS " + allList.get(opt[0]).getMeanning());
							}
							break;
						case 1:
							if ( checkedId == meaning1.getId() ) {
								...
								DisplayToast("WRONG,THE CORRECT ANSWER IS " + allList.get(opt[1]).getMeanning() );
							}
							break;
						case 2:
							if ( checkedId == meaning2.getId() ) {
								...
								DisplayToast("WRONG,THE CORRECT ANSWER IS  " + allList.get(opt[2]).getMeanning() );
							}
							break;
						case 3:
							if ( checkedId == meaning3.getId() ) {
								...
								DisplayToast("WRONG,THE CORRECT ANSWER IS  " + allList.get(opt[3]).getMeanning() );
							}
							break;
					}
